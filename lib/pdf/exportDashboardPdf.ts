...
export async function exportDashboardPdf(data: DashboardPdfData) {
  ...
  const predictions = data.resultJson.industry_predictions as {
    industry: string;
    probability: number;
  }[];
  ...
  // ---------- CURRENT SKILLS ----------
  const skills = data.inputSkills.join(", ");
  ...
  // ---------- REKOMENDASI SKILL ----------
  const recommended = data.resultJson.recommended_skills as {
    skill: string;
    score: number;
  }[];
  ...
  // ---------- LEARNING PATH ----------
  const learningPath = data.resultJson.learning_path as {
    level: string;
    skills: string[];
  }[];
